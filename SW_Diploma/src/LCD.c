...
void LCD_Command(uint8_t cs, char command)
	{
	uint16_t temp;
	if(cs==1) 	GPIO_WriteBit(GPIOB, LCD_CS1, Bit_SET);	// a cs paraméter alapján megfelelõ chipek engedélyezése
	if(cs==2) 	GPIO_WriteBit(GPIOB, LCD_CS2, Bit_SET);
	if(cs==0) GPIO_WriteBit(GPIOB, LCD_CS1 | LCD_CS2, Bit_SET); //mindkét chip engedélyezése
	temp=GPIOB->ODR & 0xFF00;						//parancsbyte kerüljön a DB0..DB7-re
	GPIOB->ODR=temp|command;
	GPIO_WriteBit(GPIOB, LCD_DI, Bit_RESET);		// parancs következik
	GPIO_WriteBit(GPIOB, LCD_EN, Bit_SET);			//EN=1
	//delay
	GPIO_WriteBit(GPIOB, LCD_EN, Bit_RESET);		//EN=0
	//delay
	GPIO_WriteBit(GPIOB, LCD_CS1 | LCD_CS2, Bit_RESET);			//chip engedélyezések visszavonása
	}
...
void LCD_Data(uint8_t cs, char data)
	...
	if(cs==1) 	GPIO_WriteBit(GPIOB, LCD_CS1, Bit_SET);	// a cs paraméter alapján megfelelõ chipek engedélyezése
	if(cs==2) 	GPIO_WriteBit(GPIOB, LCD_CS2, Bit_SET);
	if(cs==0) GPIO_WriteBit(GPIOB, LCD_CS1 | LCD_CS2, Bit_SET);

	temp=GPIOB->ODR & 0xFF00;						//parancsbyte kerüljön a DB0..DB7-re
	GPIOB->ODR=temp | data;
	GPIO_WriteBit(GPIOB, LCD_DI, Bit_SET);		// parancs következik
	GPIO_WriteBit(GPIOB, LCD_EN, Bit_SET);			//EN=1
	//delay
	GPIO_WriteBit(GPIOB, LCD_EN, Bit_RESET);		//EN=0
	//delay
...
void LCD_Character(char character, char page, char *column)		//egyes karakterekhez tartozó fonts kezdõ oszlop a képernyõn : *coulumn
{
	switch(character){
