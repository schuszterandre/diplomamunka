...
void LCD_Command(uint8_t cs, char command)
	{
	uint32_t temp;
	if(cs==1) 	HAL_GPIO_WritePin(GPIOB, LCD_CS1, GPIO_PIN_SET);	// a cs paraméter alapján megfelelõ chipek engedélyezése
	if(cs==2) 	HAL_GPIO_WritePin(GPIOB, LCD_CS2, GPIO_PIN_SET);
	if(cs==0) HAL_GPIO_WritePin(GPIOB, LCD_CS1 | LCD_CS2, GPIO_PIN_SET); //mindkét chip engedélyezése
	temp=GPIOB->ODR & 0xFFFFFF00;						//parancsbyte kerüljön a DB0..DB7-re
	GPIOB->ODR=temp|(uint32_t)command;
	HAL_GPIO_WritePin(GPIOB, LCD_DI, GPIO_PIN_RESET);		// parancs következik
	HAL_GPIO_WritePin(GPIOB, LCD_EN, GPIO_PIN_SET);			//EN=1
	HAL_Delay(2);
	HAL_GPIO_WritePin(GPIOB, LCD_EN, GPIO_PIN_RESET);		//EN=0
	//delay
	HAL_GPIO_WritePin(GPIOB, LCD_CS1 | LCD_CS2, GPIO_PIN_SET);			//chip engedélyezések visszavonása
	}
