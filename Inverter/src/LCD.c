...
#define LCD_D/I GPIO_PIN_8
#define LCD_/CS1 GPIO_PIN_10
#define LCD_/CS2 GPIO_PIN_11
...
void LCD_Line(char offset)
	{
	LCD_Write_Command(0x3F| offset);
	}

void LCD_Coloumn(char offset)
	{
	LCD_Write_Command(0x40| offset);
	}

//LCD oldal kiválasztása
void LCD_Page(char offset)
	{
	LCD_Write_Command(0xB8| offset);
	}

//LCD parancs küldése
void LCD_Write_Command(char command)
	{
	int16_t temp;									//parancsbyte kerüljön a DB0..DB7-re
	temp=GPIOB->ODR & 0xFF00;
	GPIOB->ODR=temp|command;
	GPIO_WriteBit(GPIOB, LCD_D/I, Bit_RESET);		//parancs következik
